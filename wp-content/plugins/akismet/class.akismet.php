...
class Akismet {
	...
	public static function auto_check_comment( $commentdata ) {
		...
		if ( $akismet_nonce_option == 'true' || $akismet_nonce_option == '' ) {
			...
			if ( isset( $_POST['akismet_comment_nonce'] )
                && wp_verify_nonce(
                    $_POST['akismet_comment_nonce'],
                    'akismet_comment_nonce_' . $comment['comment_post_ID']
                )
            ) {
			...
			if ( $discard ) {
				...
				if ( self::$is_rest_api_call ) {
					...
					$redirect_to = isset( $_SERVER['HTTP_REFERER'] )
                        ...
                        : ( $post ? get_permalink( $post ) : home_url() );
		        ...
		        $comment_ids = $wpdb->get_col(
		                $wpdb->prepare(
		                        "SELECT comment_id FROM {$wpdb->comments} "
                                    . " WHERE DATE_SUB(NOW(), INTERVAL %d DAY) > comment_date_gmt "
                                    . " AND comment_approved = 'spam' LIMIT %d",
                                $delete_interval,
                                $delete_limit
                        )
                )
			...
			$wpdb->query(
			        $wpdb->prepare(
			                "DELETE FROM {$wpdb->comments} "
                                . " WHERE comment_id IN ( " . $format_string . " )",
                            $comment_ids
                    )
            );
	...
	public static function delete_old_comments_meta() {
		...
		while (
		        $comment_ids = $wpdb->get_col(
		                $wpdb->prepare(
		                        "SELECT m.comment_id FROM {$wpdb->commentmeta} as m "
                                    . " INNER JOIN {$wpdb->comments} as c USING(comment_id) "
                                    . " WHERE m.meta_key = 'akismet_as_submitted' "
                                    . " AND DATE_SUB(NOW(), INTERVAL %d DAY) > c.comment_date_gmt "
                                    . " LIMIT 10000",
                                $interval
                        )
                )
	...
	public static function plugin_activation() {
		if ( version_compare( $GLOBALS['wp_version'], AKISMET__MINIMUM_WP_VERSION, '<' ) ) {
			...
			$message =
                    ...
                    .sprintf(
                            esc_html__(
                                'Akismet %s requires WordPress %s or higher.' ,
                                'akismet'
                            ),
                            AKISMET_VERSION,
                ...
                .sprintf(
                        __(
                                'Please <a href="%1$s">upgrade WordPress</a> to a current version, or <a href="%2$s">downgrade to version 2.4 of the Akismet plugin</a>.',
                                'akismet'
                        ),
